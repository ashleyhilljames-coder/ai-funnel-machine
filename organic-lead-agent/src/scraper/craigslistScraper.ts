...
export const CRAIGSLIST_HTML_SEARCH_FALLBACKS = [
  'https://lasvegas.craigslist.org/search/sks?query=water+damage+OR+pipe+burst+OR+flood+OR+leak+OR+plumber',
  'https://lasvegas.craigslist.org/search/bbb?query=water+damage+OR+pipe+burst+OR+flood+OR+leak+OR+plumber',
  'https://lasvegas.craigslist.org/search/sss?query=pipe+burst'
];

/**
 * Fallback HTML parser for Craigslist search listings if RSS endpoint returns 403 or non-XML response
 */
export function parseCraigslistHtml(html: string): CraigslistLead[] {
  const leads: CraigslistLead[] = [];
  try {
    // Extract title & link patterns from Craigslist static search html
    ...
    // Generic fallback link extractor if specific li pattern yields no matches
    if (leads.length === 0) {
...
export async function fetchCraigslistLeads(endpoints = CRAIGSLIST_RSS_ENDPOINTS): Promise<CraigslistLead[]> {
  const leads: CraigslistLead[] = [];
