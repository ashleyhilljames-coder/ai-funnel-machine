...
import { handleInboundCallWebhook } from './voice-engine/twilioWebhook.js';
import { mediaStreamWss, dashboardWss } from './voice-engine/mediaStreamServer.js';
dotenv.config();
...
const HARDCODED_CLIENT_CONFIGS: Record<string, ClientConfig>= {
 ...
 academy: {
 name: "Syncro Scale Academy",
 niche: "AI operations, sales, delivery, and system integration tutoring",
 greeting: "Welcome to the Syncro Scale Academy. I am your Syncro AI Specialist. How can I help you excel today?",
 chatGreeting: "Welcome to the Syncro Scale Academy. I am your Syncro AI Specialist. How can I help you excel today?"
 }
};
...
Once a slot is selected, call 'book_dispatch'to schedule their showing. Confirm the showing time and state that a buyer's agent will meet them at that time.`;
 ...
 if (clientId === 'academy') {
  return `You are the ultimate AI operations tutor and engineering expert for ${config.name}, operating via the web chat console.
Your goal is to assist students dynamically with absolutely anything they need across the platform. You have expert knowledge of our curriculum and architecture:
- Module 1: Core Operations & Systems. Topics include the Asynchronous Event-Driven Architecture (Zod schemas, Express routing, GCP Pub/Sub queuing, SQLite lead guarding and deduplication, Conversational Voice Agents using OpenAI Realtime and Twilio media streams, and system troubleshooting).
- Module 2: The Master Salesperson Blueprint. Topics include value propositions, lead scraping, personalized outbound cold email sequences, discovery call frameworks, and the demo close.
- Module 3: Elite Client Fulfillment & Service Delivery. Topics include client onboarding, tenant provisioning, prompt engineering, launching, analytics, and retainer maintenance.

CRITICAL: Maintain a highly intelligent, encouraging, clear, and professional tutoring tone. You can explain code, provide step-by-step help with the exercises, or answer quiz questions. Do NOT use any emojis in your response.

If the user wants to schedule a direct consulting session or discovery call with a human operations advisor:
Collect these details one-by-one:
1. Full name (accept whatever name, nickname, or alias they provide; do not query, verify, or ask if it is real, and move to the next detail)
2. Phone number
3. Email address
4. Company name (map to the 'address' parameter in 'append_lead')
5. Specific consulting or tutoring needs (map to the 'damageType' parameter in 'append_lead')

Once you have all 5 details, immediately call 'append_lead' without outputting conversational text first. Once 'append_lead' completes, tell the user you are checking available times and call 'get_available_slots'. Present the times to the user and ask which works best. Once a slot is selected, call 'book_dispatch' to schedule the call.`;
 }
...
app.post('/webhooks/inbound-call', handleInboundCallWebhook);
...
httpServer.on('upgrade', (request, socket, head) =>{
 ...
 } else if (url.pathname === '/media-stream') {
...
app.get('/api/customer-profiles', async (req: any, res: any) => {
  try {
    const sessionClientId = req.session?.clientId;
    let profiles = leadGuard.getAllCustomerProfiles();
    if (sessionClientId && sessionClientId !== 'admin') {
      const clientLeads = leadGuard.getAllRegisteredLeads(sessionClientId);
      const leadEmails = new Set(clientLeads.map(l => l.email.toLowerCase().trim()));
      profiles = profiles.filter(p => leadEmails.has(p.email.toLowerCase().trim()));
    }
...
app.get('/api/telemetry-logs', async (req: any, res: any) => {
  try {
    const sessionClientId = req.session?.clientId;
    let logs = leadGuard.getAllVoiceTelemetryLogs();
    if (sessionClientId && sessionClientId !== 'admin') {
      const clientCalls = leadGuard.getAllCallLogs(sessionClientId);
      const callSids = new Set(clientCalls.map(c => c.id));
      logs = logs.filter(l => callSids.has(l.call_sid));
    }
    res.status(200).json({ success: true, logs });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error.message });
